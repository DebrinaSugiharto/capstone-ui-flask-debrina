...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(by=['Category','App']).agg({'Reviews':'sum','Rating':'mean'}).sort_values(by='Reviews',ascending=False).reset_index().iloc[0:10].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    content_rating_order = df2.groupby('Content Rating').agg({
    'Content Rating':'count'
        }).rename({'Content Rating':'Total'}, axis=1).sort_values('Total',ascending=False)
    P = content_rating_order.axes[0]
    Q = content_rating_order['Total']
    my_colors = 'rgbkymc'
    fig = plt.figure(figsize=(8,3),dpi=300)
    fig.add_subplot()
    plt.barh(P,Q, color=my_colors)
    ...
    #kasus Debrina menjelaskan bahwa paling banyak di instal adalah untuk kategori yang bisa dikonsumsi semua orang, sementara kategori adults diatas 18tahun itu paling ga laku diinstall
